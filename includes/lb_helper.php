...
class LicenseBoxAPI{
	...
	public function verify_license($time_based_check = false, $license = false, $client = false){
		...
		if($time_based_check && $this->verification_period > 0){
			...
			$today = date('d-m-Y');
			if(empty($_SESSION["7c6051732a4d187"])){
				$_SESSION["7c6051732a4d187"] = '00-00-0000';
			}
			if($type == 1){
				$type_text = '1 day';
			}elseif($type == 3){
			...
			if(strtotime($today) >= strtotime($_SESSION["7c6051732a4d187"])){
				$get_data = $this->call_api(
					'POST',
